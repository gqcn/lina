## ADDED Requirements

### Requirement: 源码插件在构建期自动装载
系统 SHALL 支持在构建期自动发现源码插件并生成宿主可装载的静态注册表，而不要求插件作者手工修改核心源码。

#### Scenario: 扫描源码插件并生成注册表
- **WHEN** 宿主执行后端或前端构建流程
- **THEN** 宿主扫描 `apps/lina-plugins/` 下所有合法源码插件
- **AND** 生成后端静态注册表与前端插件清单
- **AND** 缺少必需入口或 manifest 不合法的插件会阻止对应注册表生成

#### Scenario: 源码插件 Go 后端参与宿主编译
- **WHEN** 一个源码插件在插件目录内提供后端 Go 入口
- **THEN** 宿主在构建期自动生成该插件的后端静态导入注册表
- **AND** 该插件的后端 Go 包与宿主后端一起编译进同一个二进制文件
- **AND** 插件作者不需要手工修改宿主核心源码来接线该插件

### Requirement: 运行时 package 与 wasm 插件可被校验和装载
系统 SHALL 支持运行时安装 `package` 与 `wasm` 产物，并在装载前完成完整性与兼容性校验。

#### Scenario: 安装 package 插件
- **WHEN** 管理员上传一个插件 `package`
- **THEN** 宿主校验清单、版本兼容性、产物摘要与资源结构
- **AND** 校验通过后提取运行时产物与前端静态资源到宿主管理目录
- **AND** 校验失败时拒绝安装并返回明确原因

#### Scenario: 安装 wasm 单文件插件
- **WHEN** 管理员上传一个单独的 `wasm` 文件
- **THEN** 宿主读取该文件中声明的插件元数据与可选资源信息
- **AND** 若插件仅声明后端能力则无需额外前端资源即可安装
- **AND** 若插件声明前端资源则宿主仅在资源可被正确提取时允许启用

### Requirement: 插件启停与升级无需重启宿主
系统 SHALL 支持在不重启宿主进程的情况下启用、禁用与升级运行时插件。

#### Scenario: 热启用插件
- **WHEN** 管理员启用一个已安装但未启用的运行时插件
- **THEN** 宿主在当前进程内加载该插件 release 并更新本地插件注册表
- **AND** 新请求可以立即访问该插件提供的页面、Hook 与治理资源
- **AND** 宿主主进程不需要重启

#### Scenario: 热升级插件
- **WHEN** 管理员将运行时插件升级到新 release
- **THEN** 宿主为新请求切换到新 release
- **AND** 已经开始处理的旧请求允许自然结束
- **AND** 正在使用该插件页面的用户会收到刷新当前页面的提示

### Requirement: 多节点以代际方式收敛插件状态
系统 SHALL 在多节点部署下通过代际同步机制传播插件变更，并避免重复迁移与双重切换。

#### Scenario: 主节点执行插件升级
- **WHEN** 多节点环境中发生插件安装、启用、禁用或升级
- **THEN** 只有被选举出来的主节点执行共享迁移与 release 切换
- **AND** 其他节点仅根据最新代际收敛本地状态
- **AND** 任一节点都可以上报其当前代际与错误状态
