...
#### Scenario: 登录成功记录
- **WHEN** 用户通过 `POST /api/auth/login` 成功登录
...
系统 SHALL 提供登录日志分页查询接口 `GET /api/loginlog`，支持按用户名、IP、状态、时间范围筛选。

#### Scenario: 分页查询登录日志
- **WHEN** 管理员请求 `GET /api/loginlog?pageNum=1&pageSize=10`
...
系统 SHALL 提供登录日志详情接口 `GET /api/loginlog/{id}`，返回完整的登录日志信息。

#### Scenario: 查看登录日志详情
- **WHEN** 管理员请求 `GET /api/loginlog/1`
- **THEN** 返回该登录日志的所有字段信息

### Requirement: 登录日志按时间范围清理
系统 SHALL 提供登录日志清理接口 `DELETE /api/loginlog/clean`，支持按时间范围硬删除日志记录。

#### Scenario: 按时间范围清理登录日志
- **WHEN** 管理员请求 `DELETE /api/loginlog/clean?beginTime=2026-01-01&endTime=2026-01-31`
- **THEN** 硬删除该时间范围内的所有登录日志记录，返回删除的记录数

#### Scenario: 清理全部登录日志
- **WHEN** 管理员请求 `DELETE /api/loginlog/clean`（不带时间参数）
- **THEN** 硬删除所有登录日志记录

### Requirement: 登录日志批量删除
系统 SHALL 提供登录日志批量删除接口 `DELETE /api/loginlog/{ids}`，支持按 ID 列表硬删除日志记录。

#### Scenario: 按 ID 批量删除
- **WHEN** 管理员请求 `DELETE /api/loginlog/1,2,3`
...
### Requirement: 登录日志导出
系统 SHALL 提供登录日志导出接口 `GET /api/loginlog/export`，按当前筛选条件导出为 xlsx 格式文件。

#### Scenario: 按筛选条件导出
- **WHEN** 管理员请求 `GET /api/loginlog/export?userName=admin&status=0`
