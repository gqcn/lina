...
// ---------------------------------------------------------------------------
// OpcodeDBQuery: read-only SQL query request / response
// ---------------------------------------------------------------------------

// HostCallDBQueryRequest carries a parameterized SELECT query.
type HostCallDBQueryRequest struct {
	SQL     string   `json:"sql"`
	Args    []string `json:"args,omitempty"`
	MaxRows int32    `json:"maxRows,omitempty"`
}

// MarshalHostCallDBQueryRequest encodes a DB query request.
func MarshalHostCallDBQueryRequest(req *HostCallDBQueryRequest) []byte {
	var content []byte
	if value := strings.TrimSpace(req.SQL); value != "" {
		content = appendStringField(content, 1, value)
	}
	for _, arg := range req.Args {
		content = appendStringField(content, 2, arg)
	}
	if req.MaxRows > 0 {
		content = appendVarintField(content, 3, uint64(req.MaxRows))
	}
	return content
}

// UnmarshalHostCallDBQueryRequest decodes a DB query request.
func UnmarshalHostCallDBQueryRequest(data []byte) (*HostCallDBQueryRequest, error) {
	out := &HostCallDBQueryRequest{}
	content := data
	for len(content) > 0 {
		fieldNumber, wireType, length := protowire.ConsumeTag(content)
		if length < 0 {
			return nil, gerror.New("解析 db query request tag 失败")
		}
		content = content[length:]
		switch fieldNumber {
		case 1:
			value, size := protowire.ConsumeString(content)
			if size < 0 {
				return nil, gerror.New("解析 db query sql 失败")
			}
			out.SQL = value
			content = content[size:]
		case 2:
			value, size := protowire.ConsumeString(content)
			if size < 0 {
				return nil, gerror.New("解析 db query arg 失败")
			}
		...
		case 3:
			value, size := protowire.ConsumeVarint(content)
			if size < 0 {
				return nil, gerror.New("解析 db query maxRows 失败")
			}
			out.MaxRows = int32(value)
			content = content[size:]
		default:
			size := protowire.ConsumeFieldValue(fieldNumber, wireType, content)
			if size < 0 {
				return nil, gerror.New("跳过未知 db query 字段失败")
			}
			content = content[size:]
		}
	}
...
// HostCallDBQueryResponse carries the result of a read-only SQL query.
type HostCallDBQueryResponse struct {
	Columns  []string   `json:"columns"`
	Rows     [][]string `json:"rows"`
	RowCount int32      `json:"rowCount"`
}

// MarshalHostCallDBQueryResponse encodes a DB query response.
func MarshalHostCallDBQueryResponse(resp *HostCallDBQueryResponse) []byte {
	var content []byte
	for _, col := range resp.Columns {
		content = appendStringField(content, 1, col)
	}
	for _, row := range resp.Rows {
		content = appendBytesField(content, 2, marshalDBRow(row))
	}
	if resp.RowCount > 0 {
		content = appendVarintField(content, 3, uint64(resp.RowCount))
	}
	return content
}

// marshalDBRow encodes a single row as repeated string fields.
func marshalDBRow(cells []string) []byte {
	var content []byte
	for _, cell := range cells {
		content = appendStringField(content, 1, cell)
	}
	return content
}

// UnmarshalHostCallDBQueryResponse decodes a DB query response.
func UnmarshalHostCallDBQueryResponse(data []byte) (*HostCallDBQueryResponse, error) {
	out := &HostCallDBQueryResponse{}
	content := data
	for len(content) > 0 {
		fieldNumber, wireType, length := protowire.ConsumeTag(content)
		if length < 0 {
			return nil, gerror.New("解析 db query response tag 失败")
		}
		content = content[length:]
		switch fieldNumber {
		case 1:
			value, size := protowire.ConsumeString(content)
			if size < 0 {
				return nil, gerror.New("解析 db query column 失败")
			}
		...
		case 2:
			value, size := protowire.ConsumeBytes(content)
			if size < 0 {
				return nil, gerror.New("解析 db query row 失败")
			}
			row, err := unmarshalDBRow(value)
			if err != nil {
				return nil, err
			}
		...
		case 3:
			value, size := protowire.ConsumeVarint(content)
			if size < 0 {
				return nil, gerror.New("解析 db query rowCount 失败")
			}
			out.RowCount = int32(value)
			content = content[size:]
		default:
			size := protowire.ConsumeFieldValue(fieldNumber, wireType, content)
			if size < 0 {
				return nil, gerror.New("跳过未知 db query response 字段失败")
			}
			content = content[size:]
		}
	}
...
// unmarshalDBRow decodes a single row from repeated string fields.
func unmarshalDBRow(data []byte) ([]string, error) {
	var cells []string
	content := data
	for len(content) > 0 {
		fieldNumber, wireType, length := protowire.ConsumeTag(content)
		if length < 0 {
			return nil, gerror.New("解析 db row tag 失败")
		}
		content = content[length:]
		switch fieldNumber {
		case 1:
			value, size := protowire.ConsumeString(content)
			if size < 0 {
				return nil, gerror.New("解析 db row cell 失败")
			}
		...
		default:
			size := protowire.ConsumeFieldValue(fieldNumber, wireType, content)
			if size < 0 {
				return nil, gerror.New("跳过未知 db row 字段失败")
			}
			content = content[size:]
		}
	}
...
// ---------------------------------------------------------------------------
// OpcodeDBExecute: write SQL request / response
// ---------------------------------------------------------------------------

// HostCallDBExecuteRequest carries a parameterized write SQL statement.
type HostCallDBExecuteRequest struct {
	SQL  string   `json:"sql"`
	Args []string `json:"args,omitempty"`
}

// MarshalHostCallDBExecuteRequest encodes a DB execute request.
func MarshalHostCallDBExecuteRequest(req *HostCallDBExecuteRequest) []byte {
	var content []byte
	if value := strings.TrimSpace(req.SQL); value != "" {
		content = appendStringField(content, 1, value)
	}
...
// UnmarshalHostCallDBExecuteRequest decodes a DB execute request.
func UnmarshalHostCallDBExecuteRequest(data []byte) (*HostCallDBExecuteRequest, error) {
	out := &HostCallDBExecuteRequest{}
	content := data
	for len(content) > 0 {
		fieldNumber, wireType, length := protowire.ConsumeTag(content)
		if length < 0 {
			return nil, gerror.New("解析 db execute request tag 失败")
		}
		content = content[length:]
		switch fieldNumber {
		case 1:
			value, size := protowire.ConsumeString(content)
			if size < 0 {
				return nil, gerror.New("解析 db execute sql 失败")
			}
			out.SQL = value
			content = content[size:]
		case 2:
			value, size := protowire.ConsumeString(content)
			if size < 0 {
				return nil, gerror.New("解析 db execute arg 失败")
			}
			out.Args = append(out.Args, value)
			content = content[size:]
		default:
			size := protowire.ConsumeFieldValue(fieldNumber, wireType, content)
			if size < 0 {
				return nil, gerror.New("跳过未知 db execute 字段失败")
			}
			content = content[size:]
		}
	}
...
// HostCallDBExecuteResponse carries the result of a write SQL statement.
type HostCallDBExecuteResponse struct {
	RowsAffected int64 `json:"rowsAffected"`
	LastInsertID int64 `json:"lastInsertId"`
}

// MarshalHostCallDBExecuteResponse encodes a DB execute response.
func MarshalHostCallDBExecuteResponse(resp *HostCallDBExecuteResponse) []byte {
	var content []byte
	if resp.RowsAffected > 0 {
		content = appendVarintField(content, 1, uint64(resp.RowsAffected))
	}
	if resp.LastInsertID > 0 {
		content = appendVarintField(content, 2, uint64(resp.LastInsertID))
	}
	return content
}

// UnmarshalHostCallDBExecuteResponse decodes a DB execute response.
func UnmarshalHostCallDBExecuteResponse(data []byte) (*HostCallDBExecuteResponse, error) {
	out := &HostCallDBExecuteResponse{}
	content := data
	for len(content) > 0 {
		fieldNumber, wireType, length := protowire.ConsumeTag(content)
		if length < 0 {
			return nil, gerror.New("解析 db execute response tag 失败")
		}
		content = content[length:]
		switch fieldNumber {
		case 1:
			value, size := protowire.ConsumeVarint(content)
			if size < 0 {
				return nil, gerror.New("解析 db execute rowsAffected 失败")
			}
			out.RowsAffected = int64(value)
			content = content[size:]
		case 2:
			value, size := protowire.ConsumeVarint(content)
			if size < 0 {
				return nil, gerror.New("解析 db execute lastInsertId 失败")
			}
			out.LastInsertID = int64(value)
			content = content[size:]
		default:
			size := protowire.ConsumeFieldValue(fieldNumber, wireType, content)
			if size < 0 {
				return nil, gerror.New("跳过未知 db execute response 字段失败")
			}
			content = content[size:]
		}
	}
