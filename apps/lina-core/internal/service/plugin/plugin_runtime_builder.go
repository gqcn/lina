...
// source directories so sample runtime plugins no longer need to commit binary output.
...
func (s *Service) BuildRuntimeWasmArtifactFromSource(pluginDir string) (*RuntimeBuildOutput, error) {
	...
	if err := s.loadPluginYAMLFile(manifestPath, manifest); err != nil {
		return nil, gerror.Wrap(err, "failed to load runtime plugin manifest")
	}
	...
	return &RuntimeBuildOutput{
		ArtifactPath: filepath.Join(pluginDir, buildPluginRuntimeBuildOutputRelativePath(manifest.ID)),
		Content:      content,
		Manifest:     manifest,
	}, nil
}
...
func buildPluginRuntimeBuildOutputRelativePath(pluginID string) string {
	return filepath.Join("temp", buildPluginRuntimeArtifactFileName(pluginID))
}

func validateRuntimeBuildManifest(manifest *pluginManifest, manifestPath string) error {
	if manifest == nil {
		return gerror.New("runtime plugin manifest cannot be nil")
	}
	if strings.TrimSpace(manifest.ID) == "" {
		return gerror.Newf("runtime plugin manifest missing id: %s", manifestPath)
	}
	if strings.TrimSpace(manifest.Name) == "" {
		return gerror.Newf("runtime plugin manifest missing name: %s", manifestPath)
	}
	if strings.TrimSpace(manifest.Version) == "" {
		return gerror.Newf("runtime plugin manifest missing version: %s", manifestPath)
	}
	manifest.Type = normalizePluginType(manifest.Type).String()
	if normalizePluginType(manifest.Type) != pluginTypeRuntime {
		return gerror.Newf("runtime sample manifest type must be runtime: %s", manifestPath)
	}
	if !pluginManifestIDPattern.MatchString(manifest.ID) {
		return gerror.Newf("runtime plugin id must use kebab-case: %s", manifest.ID)
	}
	if err := validatePluginManifestSemanticVersion(manifest.Version); err != nil {
		return gerror.Wrapf(err, "runtime plugin version is invalid: %s", manifestPath)
	}
	return nil
}

func (s *Service) collectRuntimeBuilderFrontendAssets(pluginDir string) ([]*pluginRuntimeArtifactFrontendAsset, error) {
	...
	sort.Strings(files)
	assets := make([]*pluginRuntimeArtifactFrontendAsset, 0, len(files))
	for _, filePath := range files {
		...
		assets = append(assets, &pluginRuntimeArtifactFrontendAsset{
			Path:          normalizedPath,
...
func (s *Service) collectRuntimeBuilderSQLAssets(
	...
	assets := make([]*pluginRuntimeArtifactSQLAsset, 0, len(relativePaths))
	for _, relativePath := range relativePaths {
		...
		assets = append(assets, &pluginRuntimeArtifactSQLAsset{
...
func buildRuntimeWasmArtifactContent(
	...
	bytes = appendWasmCustomSection(bytes, pluginRuntimeWasmSectionManifest, manifestPayload)
	bytes = appendWasmCustomSection(bytes, pluginRuntimeWasmSectionRuntime, runtimePayload)
	...
	if len(frontendAssets) > 0 {
		...
		bytes = appendWasmCustomSection(bytes, pluginRuntimeWasmSectionFrontend, payload)
	}
	if len(installSQLAssets) > 0 {
		...
		bytes = appendWasmCustomSection(bytes, pluginRuntimeWasmSectionInstallSQL, payload)
	}
	if len(uninstallSQLAssets) > 0 {
		...
		bytes = appendWasmCustomSection(bytes, pluginRuntimeWasmSectionUninstallSQL, payload)
	}
	if len(hookSpecs) > 0 {
		...
		bytes = appendWasmCustomSection(bytes, pluginRuntimeWasmSectionBackendHooks, payload)
	}
	if len(resourceSpecs) > 0 {
		...
		bytes = appendWasmCustomSection(bytes, pluginRuntimeWasmSectionBackendRes, payload)
	}
	return bytes, nil
}
