// This file resolves runtime plugin frontend assets from in-memory bundles that
// are built from the runtime wasm artifact stored in plugin.runtime.storagePath.
...
// enabled runtime plugins during host startup. Request-time resolution still
...
func (s *Service) PrewarmRuntimeFrontendBundles(ctx context.Context) error {
	...
	for _, manifest := range manifests {
		...
		if registryErr != nil {
			failures = append(
				failures,
				gerror.Wrapf(registryErr, "预热运行时插件前端资源失败: %s", manifest.ID).Error(),
			)
			continue
		}
...
// ResolveRuntimeFrontendAsset resolves one enabled runtime plugin frontend asset for public serving.
func (s *Service) ResolveRuntimeFrontendAsset(
	...
	if normalizePluginType(manifest.Type) != pluginTypeRuntime {
		return nil, gerror.New("当前插件不是运行时插件")
	}
	if manifest.RuntimeArtifact == nil || len(manifest.RuntimeArtifact.FrontendAssets) == 0 {
		return nil, gerror.New("当前运行时插件未声明前端资源")
	}
	if strings.TrimSpace(version) == "" || version != manifest.Version {
		return nil, gerror.New("当前运行时插件版本不存在或已切换")
	}
	...
	if registry == nil || registry.Installed != pluginInstalledYes || registry.Status != pluginStatusEnabled {
		return nil, gerror.New("当前运行时插件未启用")
	}
