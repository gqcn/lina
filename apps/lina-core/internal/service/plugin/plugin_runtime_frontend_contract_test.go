package plugin

import (
	"context"
	"encoding/base64"
	"path/filepath"
	"strings"
	"testing"

	"lina-core/internal/model/entity"
)

func TestValidateRuntimeHostedMenuBindingsAcceptsHostedRuntimeModes(t *testing.T) {
	service := New()
	resetRuntimeFrontendBundleCache()
	t.Cleanup(resetRuntimeFrontendBundleCache)
	pluginDir := createTestRuntimePluginDirWithFrontendAssets(
		t,
		"plugin-runtime-bindings",
		"Runtime Binding Plugin",
		"v0.3.0",
		[]*pluginRuntimeArtifactFrontendAsset{
			{
				Path:          "index.html",
				ContentBase64: base64.StdEncoding.EncodeToString([]byte("<html><body>hosted entry</body></html>")),
				ContentType:   "text/html; charset=utf-8",
			},
			{
				Path:          "mount.js",
				ContentBase64: base64.StdEncoding.EncodeToString([]byte("export function mount() {}")),
				ContentType:   "application/javascript",
			},
		},
		nil,
		nil,
	)

	manifest := &pluginManifest{
		ID:           "plugin-runtime-bindings",
		Name:         "Runtime Binding Plugin",
		Version:      "v0.3.0",
		Type:         pluginTypeRuntime.String(),
		ManifestPath: filepath.Join(pluginDir, "plugin.yaml"),
		RootDir:      pluginDir,
	}
	if err := service.validatePluginManifest(manifest, manifest.ManifestPath); err != nil {
		t.Fatalf("expected runtime manifest to be valid, got error: %v", err)
	}

	hostedBaseURL := service.BuildRuntimeFrontendPublicBaseURL(manifest.ID, manifest.Version)
	menus := []*entity.SysMenu{
		{
			MenuKey: "plugin:plugin-runtime-bindings:iframe-entry",
			Name:    "Hosted iframe entry",
			Path:    hostedBaseURL + "index.html",
			IsFrame: 0,
		},
		{
			MenuKey: "plugin:plugin-runtime-bindings:new-window-entry",
			Name:    "Hosted new window entry",
			Path:    hostedBaseURL + "index.html",
			IsFrame: 1,
		},
		{
			MenuKey:    "plugin:plugin-runtime-bindings:embedded-entry",
			Name:       "Hosted embedded entry",
			Path:       hostedBaseURL + "mount.js",
			Component:  pluginRuntimePageComponentPath,
			QueryParam: `{"pluginAccessMode":"embedded-mount"}`,
			IsFrame:    0,
		},
	}

	if err := service.validateRuntimeHostedMenuBindings(context.Background(), manifest, menus); err != nil {
		t.Fatalf("expected runtime hosted menu bindings to be valid, got error: %v", err)
	}
}

func TestValidateRuntimeHostedMenuBindingsRejectsBrokenEmbeddedMountContract(t *testing.T) {
	service := New()
	resetRuntimeFrontendBundleCache()
	t.Cleanup(resetRuntimeFrontendBundleCache)
	pluginDir := createTestRuntimePluginDirWithFrontendAssets(
		t,
		"plugin-runtime-broken-bindings",
		"Broken Runtime Binding Plugin",
		"v0.3.1",
		[]*pluginRuntimeArtifactFrontendAsset{
			{
				Path:          "index.html",
				ContentBase64: base64.StdEncoding.EncodeToString([]byte("<html><body>hosted entry</body></html>")),
				ContentType:   "text/html; charset=utf-8",
			},
		},
		nil,
		nil,
	)

	manifest := &pluginManifest{
		ID:           "plugin-runtime-broken-bindings",
		Name:         "Broken Runtime Binding Plugin",
		Version:      "v0.3.1",
		Type:         pluginTypeRuntime.String(),
		ManifestPath: filepath.Join(pluginDir, "plugin.yaml"),
		RootDir:      pluginDir,
	}
	if err := service.validatePluginManifest(manifest, manifest.ManifestPath); err != nil {
		t.Fatalf("expected runtime manifest to be valid, got error: %v", err)
	}

	hostedBaseURL := service.BuildRuntimeFrontendPublicBaseURL(manifest.ID, manifest.Version)
	menus := []*entity.SysMenu{
		{
			MenuKey:    "plugin:plugin-runtime-broken-bindings:embedded-entry",
			Name:       "Broken embedded entry",
			Path:       hostedBaseURL + "index.html",
			Component:  pluginRuntimePageComponentPath,
			QueryParam: `{"pluginAccessMode":"embedded-mount"}`,
			IsFrame:    0,
		},
	}

	err := service.validateRuntimeHostedMenuBindings(context.Background(), manifest, menus)
	if err == nil {
		t.Fatalf("expected broken embedded mount contract to be rejected")
	}
	if expected := ".js 或 .mjs"; !strings.Contains(err.Error(), expected) {
		t.Fatalf("expected error to mention %q, got: %v", expected, err)
	}
}
