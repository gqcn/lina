...
// the runtime wasm artifact stored in plugin.runtime.storagePath. The wasm
...
func buildRuntimeFrontendBundle(manifest *pluginManifest) (*runtimeFrontendBundle, error) {
	...
	if manifest.RuntimeArtifact == nil {
		return nil, gerror.New("当前运行时插件缺少有效产物")
	}
	...
	for _, asset := range manifest.RuntimeArtifact.FrontendAssets {
		...
		if asset.Path == "" {
			return nil, gerror.New("当前运行时插件前端资源路径不能为空")
		}
		if len(asset.Content) == 0 {
			return nil, gerror.Newf("当前运行时插件前端资源内容为空: %s", asset.Path)
		}
...
func (b *runtimeFrontendBundle) ReadAsset(relativePath string) ([]byte, string, error) {
	if b == nil || b.FileSystem == nil {
		return nil, "", gerror.New("当前运行时插件前端资源不可用")
	}
	...
	if err != nil {
		return nil, "", gerror.New("当前运行时插件前端资源不存在")
	}
