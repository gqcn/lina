...
// RuntimeUploadInput defines input for uploading a runtime wasm package.
...
// RuntimeUploadOutput defines output for uploading a runtime wasm package.
type RuntimeUploadOutput struct {
...
// UploadRuntimePackage validates one runtime wasm package and writes it into plugin.runtime.storagePath.
func (s *Service) UploadRuntimePackage(ctx context.Context, in *RuntimeUploadInput) (*RuntimeUploadOutput, error) {
	if in == nil || in.File == nil {
		return nil, gerror.New("请上传运行时插件文件")
	}

	source, err := in.File.Open()
	if err != nil {
		return nil, gerror.Wrap(err, "打开运行时插件文件失败")
	}
	defer source.Close()

	content, err := io.ReadAll(source)
	if err != nil {
		return nil, gerror.Wrap(err, "读取运行时插件文件失败")
	}
	if len(content) == 0 {
		return nil, gerror.New("运行时插件文件不能为空")
	}
	return s.storeUploadedRuntimePackage(
		ctx,
		normalizeRuntimeUploadFilename(in.File.Filename),
		content,
		in.OverwriteSupport,
	)
}

func normalizeRuntimeUploadFilename(filename string) string {
...
func (s *Service) storeUploadedRuntimePackage(
	...
	targetPath := filepath.Join(storageDir, buildPluginRuntimeArtifactFileName(manifest.ID))
	...
	if registry != nil && normalizePluginType(registry.Type) != pluginTypeRuntime {
		return nil, gerror.New("已存在同名源码插件，不允许上传运行时插件覆盖")
	}
	if registry != nil && registry.Installed == pluginInstalledYes {
		return nil, gerror.New("已安装的运行时插件暂不支持通过上传覆盖，请先卸载后再重新上传")
	}
	if conflictPath, conflictErr := s.findDuplicateRuntimeArtifactPath(storageDir, manifest.ID, targetPath); conflictErr != nil {
		...
		return nil, gerror.Newf("运行时插件目录存在重复的插件ID %s，请先移除冲突文件: %s", manifest.ID, conflictPath)
	}
	if gfile.Exists(targetPath) && !overwriteSupport {
		return nil, gerror.New("运行时插件文件已存在，请开启覆盖后重试")
	}
	if err = gfile.Mkdir(storageDir); err != nil {
		return nil, gerror.Wrap(err, "创建运行时插件存储目录失败")
	}
	...
	if err = gfile.PutBytes(targetPath, content); err != nil {
		return nil, gerror.Wrap(err, "写入运行时插件产物失败")
	}
...
func (s *Service) findDuplicateRuntimeArtifactPath(storageDir string, pluginID string, targetPath string) (string, error) {
	...
	for _, artifactPath := range artifactFiles {
		...
		if parseErr != nil {
			return "", gerror.Wrapf(parseErr, "解析现有运行时插件文件失败: %s", artifactPath)
		}
...
func (s *Service) validateUploadedRuntimeManifest(manifest *pluginManifest) error {
	if manifest == nil {
		return gerror.New("运行时插件清单不能为空")
	}
	manifest.Type = normalizePluginType(manifest.Type).String()
	if manifest.Type != pluginTypeRuntime.String() {
		return gerror.New("运行时插件类型必须是 runtime")
	}
	if manifest.ID == "" || !pluginManifestIDPattern.MatchString(manifest.ID) {
		return gerror.New("运行时插件 ID 非法")
	}
	if manifest.Name == "" {
		return gerror.New("运行时插件名称不能为空")
	}
