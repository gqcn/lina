// This file hosts the low-level wazero bridge used by dynamic route execution.
...
// InvalidateWasmModuleCache removes the cached compiled module for the given
// artifact path. This must be called when a plugin's active release changes
// (upgrade, rollback, uninstall) so subsequent requests recompile from the
// new artifact.
func InvalidateWasmModuleCache(artifactPath string) {
...
// InvalidateAllWasmModuleCache removes all cached compiled modules. This is
// useful during full reconciliation passes or shutdown.
func InvalidateAllWasmModuleCache() {
	wasmModuleCacheMu.Lock()
	defer wasmModuleCacheMu.Unlock()
...
// executeDynamicWasmBridge runs one bridge invocation against the archived
// active Wasm artifact using the alloc -> write -> execute -> read protocol
// defined by the shared bridge ABI.
func (s *Service) executeDynamicWasmBridge(
	ctx context.Context,
	...
	rt, compiled, err := getOrCompileWasmModule(ctx, manifest.RuntimeArtifact.Path)
	if err != nil {
		return nil, err
	}
	...
	// plugin identity, capabilities, and host services.
	...
	allocFn := module.ExportedFunction(manifest.BridgeSpec.AllocExport)
	executeFn := module.ExportedFunction(manifest.BridgeSpec.ExecuteExport)
