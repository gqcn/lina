// This file validates how runtime plugin menus consume hosted frontend assets.
...
const (
	pluginRuntimeHostedAssetURLPrefix     = "/plugin-assets/"
	pluginRuntimePageComponentPath        = "system/plugin/runtime-page"
	pluginRuntimeMenuQueryKeyAccessMode   = "pluginAccessMode"
	pluginRuntimeMenuAccessModeEmbedded   = "embedded-mount"
	pluginRuntimeMenuEmbeddedJSExtension  = ".js"
	pluginRuntimeMenuEmbeddedMJSExtension = ".mjs"
)

// ValidateRuntimeFrontendMenuBindings verifies that runtime plugin menus only
...
func (s *Service) resolveRuntimeHostedMenuAssetPath(
	...
	if !strings.HasPrefix(normalizedPath, pluginRuntimeHostedAssetURLPrefix) {
		return "", false, nil
	}
...
func validateRuntimeHostedMenuMode(
	...
	accessMode := strings.TrimSpace(queryParams[pluginRuntimeMenuQueryKeyAccessMode])
	isEmbeddedComponent := componentPath == pluginRuntimePageComponentPath

	if accessMode == pluginRuntimeMenuAccessModeEmbedded {
		if !isEmbeddedComponent {
			return gerror.Newf(
				"宿主内嵌挂载菜单必须使用组件 %s",
				pluginRuntimePageComponentPath,
			)
		}
		if menu.IsFrame != 0 {
		...
		if extension != pluginRuntimeMenuEmbeddedJSExtension && extension != pluginRuntimeMenuEmbeddedMJSExtension {
	...
	if isEmbeddedComponent {
		return gerror.Newf(
			...
			pluginRuntimePageComponentPath,
			pluginRuntimeMenuQueryKeyAccessMode,
			pluginRuntimeMenuAccessModeEmbedded,
		)
	}
	return nil
}
