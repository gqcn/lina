...
const pluginDynamicRoutePublicPrefix = "/api/v1/extensions"
...
// DispatchDynamicRoute dispatches one fixed-prefix request into the active
// release of one dynamic plugin. Matching always happens against the archived
// active manifest so staged uploads cannot affect live traffic before reconcile.
...
func (s *Service) matchDynamicRoute(ctx context.Context, request *ghttp.Request) (*dynamicRouteMatch, error) {
	...
	if !strings.HasPrefix(publicPath, pluginDynamicRoutePublicPrefix+"/") {
		return nil, nil
	}
	pathSuffix := strings.TrimPrefix(publicPath, pluginDynamicRoutePublicPrefix+"/")
	...
	manifest, err := s.getActivePluginManifest(ctx, pluginID)
	if err != nil {
		return nil, nil
	}
...
func (s *Service) prepareDynamicRouteRuntime(
	...
	manifest, err := s.getActivePluginManifest(ctx, match.PluginID)
	if err != nil {
		return nil, pluginbridge.NewNotFoundResponse(err.Error()), nil
	}
	registry, err := s.getPluginRegistry(ctx, match.PluginID)
	if err != nil {
		return nil, nil, err
	}
...
// buildDynamicRouteRequestEnvelope turns the host HTTP request into the stable
// bridge DTO that guest code understands and injects host-owned auth context.
// This method is used by DispatchDynamicRoute (the non-middleware direct-call
// path) where no prior middleware has resolved the identity snapshot, so it
// performs its own auth check internally.
func (s *Service) buildDynamicRouteRequestEnvelope(
	ctx context.Context,
	manifest *pluginManifest,
	match *dynamicRouteMatch,
	request *ghttp.Request,
) (*pluginbridge.BridgeRequestEnvelopeV1, *pluginbridge.BridgeResponseEnvelopeV1, error) {
	if request == nil || match == nil || match.Route == nil {
		return nil, pluginbridge.NewBadRequestResponse("Dynamic route context is incomplete"), nil
	}

	var identity *pluginbridge.IdentitySnapshotV1
	if match.Route.Access == pluginbridge.AccessLogin {
		// Login-protected routes receive a host-verified identity snapshot so guest
		// code can focus on business logic instead of reimplementing auth checks.
		snapshot, failure, err := s.buildDynamicRouteIdentitySnapshot(ctx, match, request)
		if err != nil {
			return nil, nil, err
		}
		if failure != nil {
	...
	envelope, err := s.buildDynamicRouteRequestEnvelopeWithIdentity(ctx, manifest, match, request, identity)
	if err != nil {
		return nil, nil, err
	}
...
// buildDynamicRouteIdentitySnapshot validates session state and permission
// grants on the host side before forwarding the request into guest code.
func (s *Service) buildDynamicRouteIdentitySnapshot(
	...
	s.bizCtxSvc.SetUser(ctx, claims.TokenId, claims.UserId, claims.Username, claims.Status)
	s.DispatchAfterAuthRequest(
		ctx,
...
func (s *Service) getDynamicRoutePermissionsByRoleIDs(ctx context.Context, roleIDs []int) ([]string, error) {
	...
	menuItems = s.FilterPermissionMenus(ctx, menuItems)
...
// writeDynamicRouteResponse writes the guest response back without going
// through GoFrame's default success wrapper, otherwise raw plugin payloads
// would be polluted by host-managed response formatting.
