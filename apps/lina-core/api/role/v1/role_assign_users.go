...
type RoleAssignUsersReq struct {
	g.Meta  `path:"/role/:id/users" method:"post" summary:"分配用户到角色" tags:"角色管理" dc:"批量分配用户到指定角色，已分配的用户不会被重复添加"`
	Id      int    `p:"id" v:"required|min:1" dc:"角色ID" eg:"1"`
	UserIds []int  `p:"userIds" v:"required|min:1" dc:"要分配的用户ID列表" eg:"[2,3,4]"`
}
