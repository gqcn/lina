...
// Role Menu Tree API - get menus with checked keys for a specific role

type RoleMenuTreeReq struct {
	g.Meta `path:"/menu/role/:roleId" method:"get" tags:"菜单管理" summary:"获取角色的菜单树" dc:"获取角色的菜单树，用于角色编辑时显示已分配的菜单。返回所有菜单树和该角色已分配的菜单ID列表"`
