...
type ExcludeReq struct {
	g.Meta `path:"/dept/exclude/{id}" method:"get" tags:"部门管理" summary:"获取排除节点后的部门列表" dc:"获取排除指定部门及其所有子部门后的部门列表，主要用于编辑部门时选择父级部门的下拉列表，防止将部门的父级设置为自身或其子部门造成循环引用" permission:"system:dept:query"`
