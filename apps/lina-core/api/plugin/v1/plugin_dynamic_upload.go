...
type UploadDynamicPackageReq struct {
	g.Meta           `path:"/plugins/dynamic/package" method:"post" mime:"multipart/form-data" tags:"插件管理" summary:"上传动态插件包" dc:"上传一个 dynamic wasm 动态插件包到 plugin.dynamic.storagePath。宿主会校验 wasm 文件头、自定义节、嵌入清单、ABI 版本和可选嵌入 SQL 资源，然后将产物规范化写入 <storagePath>/<plugin-id>.wasm；当前已安装的动态插件暂不支持直接通过上传覆盖升级"`
