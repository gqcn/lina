...
type PluginItem struct {
	...
	Type        string `json:"type" dc:"插件一级类型：source=源码插件 runtime=运行时插件" eg:"source"`
	...
	StatusKey   string `json:"statusKey" dc:"插件状态在系统插件注册表中的定位键名，前端注册表监听会使用该键判断是否需要刷新插件状态" eg:"sys_plugin.status:plugin-demo"`
