...
type ResourceListReq struct {
	g.Meta   `path:"/plugins/{id}/resources/{resource}" method:"get" tags:"插件管理" summary:"查询插件资源数据" dc:"按插件通用资源契约查询插件自有的后端资源数据，资源的字段、过滤条件与排序规则均由插件目录下的后端实现注册定义"`
	Id       string `json:"id" v:"required|length:1,64" dc:"插件唯一标识" eg:"plugin-demo"`
	Resource string `json:"resource" v:"required|length:1,64" dc:"插件资源标识，由插件自身在插件目录后端实现中注册" eg:"login-audits"`
