...
当前后端能力已支持把插件目录内的 Go 源码与宿主一起编译；`plugin-demo` 通过 `backend/plugin.go` 在编译期注册自己的 Hook 与资源能力。当前前端能力也已支持把插件目录内的 Vue 页面与 Slot 源码纳入宿主构建，由宿主在运行时装载。这是一阶段源码插件的最小接入方式，后续再演进到更完整的 runtime `package/wasm` 机制。
...
- 登录审计 Hook 与资源接口继续保留在后端，用于验证插件后端能力，但不再提供独立前端页面入口
