...
export class MenuPage {
  ...
  async createRootMenu(params: {
    ...
    const parentSelect = drawer.locator('.ant-select:not(.ant-select-hidden)').first();
    ...
    if (params.type !== 'D') {
      ...
      await drawer.locator('.ant-radio-button-wrapper').filter({ hasText: typeLabels[params.type] }).click();
      await this.page.waitForTimeout(500);
    }
    ...
    if (params.sort !== undefined) {
      const sortInput = this.drawer.locator('input[role="spinbutton"], .ant-input-number-input').first();
  ...
  async searchMenu(name: string) {
    const searchInput = this.page.locator(
      '.vxe-grid--form input[placeholder="请输入菜单名称"]',
    );
